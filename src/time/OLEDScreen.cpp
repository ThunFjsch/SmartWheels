...
void initIOModule(){
	//u8g2_Setup_ssd1306_i2c_128x32_univision_f(&u8g2, U8G2_R0, u8x8_byte_avr_hw_i2caadasd, u8x8_avr_delay);
...
int main (void)
{
	...
	PORTB |= (1<<DDB0);
	while(1){
		drawDisplay(state, speed, directionForwBack, directionTurn);
		if (!(PINB & (1 << PINB0))){
			state++;
			if (state > 2){
				state = 0;
