...
uint32_t getDistance(){
	...
	//	if(!(first_reading == 0 && second_reading == 0) && !(first_reading != 0 && second_reading == 0)){
	//	hasDistanceCalced = true;	
	//}
	return distance_cm;
}
...
ISR(TIMER1_CAPT_vect){
	...
	if (first_reading != 0 && second_reading != 0){// && hasDistanceCalced
		duty_cycle =  second_reading - first_reading;
		//	hasDistanceCalced = false;
		getDistance();
		first_reading = 0;
		second_reading = 0;
	}
